...
interface QuizFormContextProps {
  removeQuiz: (currentQuizId: number) => void;
  quiz: Quiz;
...
const QuizFormContext = React.createContext<QuizFormContextProps | null>(null);
...
export const QuizFormProvider = ({
  ...
  const quiz = getQuizOrCreateNew(quizId ? parseInt(quizId) : Date.now(), quizzes);
...
export const useQuizFormContext = () => {
  const context = React.useContext(QuizFormContext);
  if (!context) {
